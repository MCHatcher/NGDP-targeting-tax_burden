//Dynare code for robustness analys with respect to the shock variances (Figs 7-8). 
...
//-----------------------------------------------
n_loop_vol = 8;
sigma_stack = linspace(0,0.075,n_loop_vol);
Lambda_vol = NaN(n_loop_vol,1); Stack_tau_NIT = Lambda_vol; var_tau_NIT = Lambda_vol; Index_max = Lambda_vol;
...
for m=1:n_loop_vol
    ...
    n_loop = 500; 
    //chi_stack = linspace(0.865*(1+n),0.955*(1+n),n_loop); //For IT-NIT comparison
    chi_stack = linspace(0.899*(1+n),0.909*(1+n),n_loop); //For IT-NIT comparison
    ...
    if dummy_IT == 1
        ...
        Stack_tau_outer_IT(m) = oo_.mean(12);
        var_tau_outer_IT(m) = oo_.var(12,12);
        U_lump(m) = oo_.mean(25);
        Lambda_TB_IT(m) = 100*( (U_lump(m)/Stack_utility_outer_IT(m))^(1/thetta*(1-gama)) - 1);  //Consumption equiv. welfare gain 
    else
        Stack_utility_outer(m) = oo_.mean(13); 
        Stack_tau_NIT(m) = oo_.mean(12)/Stack_tau_outer_IT(m);
        var_tau_NIT(m) = oo_.var(12,12)/var_tau_outer_IT(m);
        Lambda_TB_NIT(m) = 100*( (U_lump(m)/Stack_utility_outer(m))^(1/thetta*(1-gama)) - 1);  //Consumption equiv. welfare gain
        TB_NIT(m) = Lambda_TB_NIT(m) - Lambda_TB_IT(m); 
         
        Lambda_vol(m) = 100*( (Stack_utility_outer(m)/Stack_utility_outer_IT(m))^(1/thetta*(1-gama)) - 1);  //Consumption equiv. welfare gain 
    end
...
if dummy_IT == 0
    figure(1)
    hold on, subplot(1,2,2), plot(sigma_stack, Lambda_vol,'--k','LineWidth', 1),
    //title('Welfare loss vs real risk'), hold on, xlabel('Std(TFP innovation), \sigma_A'), ylabel('% cons. equiv.')
    title('Welfare loss vs nominal risk'), hold on, xlabel('Std(control error), \sigma_e'), ylabel('% cons. equiv.')

    figure(2)
    //subplot(1,2,1), hold on, plot(sigma_stack, Stack_tau_NIT,'k','LineWidth', 1), 
    //title('Mean taxes: $E[\tau_t]$'), hold on, xlabel('Standard deviation'), ylabel('Ratio')
    subplot(1,2,2), hold on, plot(sigma_stack, var_tau_NIT,'k','LineWidth', 1)
    title('Variance of taxes: $var[\tau_t]$'), hold on, xlabel('Standard deviation'), ylabel('Ratio')
